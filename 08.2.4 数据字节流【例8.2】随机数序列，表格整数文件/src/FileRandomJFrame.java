...
//        new FileRandomJFrame("random.int");         //参数是文件名，当前文件夹；若无文件，写时创建 
        new FileRandomJFrame("F:\\Java\\JavaYeheya\\08.2.4 数据字节流【例8.2】随机数序列，表格整数文件\\random.int");   //文件路径及文件名正确
...
/*
研究，以下第5版教材没写。
① 也可采用对象字节流，将Integer存储成对象文件，算法同本例数据流，不是通用方法，未写。
②【第4版例8.5】写readFromText()、writeToText()方法，采用字符流保存整数，不合适，
不符合就近原则，不提倡，第5版教材没写。详见试题库。
 */
//@author：Yeheya，2017年8月24日，2017年10月14日，2017年11月17日，2018年7月27日